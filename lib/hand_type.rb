module HandType
  ...
  def royal_flush
    ...
    return :royal_flush if suits.all?{|suit| suit == suits[0]} && sorted_cards.last.face_value == :ace
  end
  ...
  def straight_flush
    ...
    suits = @cards.map(&:suit)
    return :straight_flush if suits.all?{|suit| suit == suits[0]} && sorted_cards.last.face_value != :ace
  end
  ...
  def flush
    suits = @cards.map(&:suit)
    return :flush if suits.all?{|suit| suit == suits[0]} && self.straight.nil?
  end
